...
public class HomeController
{
    ...
    private void loadAndDisplayProjects()
    {
        try
        {
            ...
            projects.addAll(collaboratorProjects.stream().filter(p -> !projects.contains(p)).toList());
     ...
     */
    @FXML private void createNewProject()
    {
        try
        {
    ...
    private void showInviteDialog(Project project)
    {
        ...
        sendButton.setOnAction(e -> {
            ...
            else
            {
                ...
                {
                    // Send the invitation
                    projectFacade.inviteUser(project.getProjectId(),
                        invitedUsername,
                        currentSession.getUsername(),
                        fr.opal.type.EPermission.CONTRIBUTOR);
     ...
     */
    @FXML private void openInvitations()
    {
        try
        {
            // Get pending invitations for current user
            List<ProjectInvitation> pendingInvitations =
                projectFacade.getPendingInvitations(currentSession.getUsername());
            ...
            else
            {
                ...
                for (ProjectInvitation invitation : pendingInvitations)
                {
                    ...
                    invitationCard.setStyle(
                        "-fx-border-color: #ddd; -fx-border-radius: 5; -fx-padding: 15; -fx-background-color: #f9f9f9;");
                    ...
                    acceptBtn.setStyle(
                        "-fx-padding: 8 15; -fx-font-size: 12; -fx-background-color: #4CAF50; -fx-text-fill: white;");

                    javafx.scene.control.Button declineBtn = new javafx.scene.control.Button("✗ Decline");
                    declineBtn.setStyle(
                        "-fx-padding: 8 15; -fx-font-size: 12; -fx-background-color: #f44336; -fx-text-fill: white;");
            ...
            if (currentSession != null && notificationsBtn != null)
            {
                ...
                if (unreadCount > 0)
                {
                    notificationsBtn.setStyle(
                        "-fx-font-size: 12; -fx-background-color: #e74c3c; -fx-text-fill: white; -fx-font-weight: bold;");
                }
                else
                {
                    notificationsBtn.setStyle(
                        "-fx-font-size: 12; -fx-background-color: #95a5a6; -fx-text-fill: white;");
                }
            }
        }
        catch (Exception e)
    ...
    @FXML private void openNotifications()
    {
        try
        {
            ...
            dialog.getDialogPane().getStylesheets().add(
                getClass().getResource("/fr/opal/notification.css").toExternalForm());
            ...
            dialog.getDialogPane().lookupAll(".button").forEach(node -> {
                if (node instanceof javafx.scene.control.Button)
                {
                    ...
                    btn.setStyle("-fx-background-color: " + accentColor + "; "
